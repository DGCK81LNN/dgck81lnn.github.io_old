  ...
  window.vm = new Vue({
    ...
    methods: {
      handleInput(raw, offset) {
        vm.code = raw;
        let frag = document.createDocumentFragment(), line = document.createElement("div"), token = null, text = null;
        let i = 0, l = raw.length, depth2 = 0, error = false, newLine = true;
        ...
        while (i < l) {
          ...
          if (char === "\n") {
            if (newLine) {
              text = document.createElement("br"), line.appendChild(text);
            }
            newLine = true;
            ...
            if (char === "+")
              ...
              depth2++, token.classList.add("branching", "important");
            else if (char === "]") {
              depth2--, token.classList.add("branching", "important");
              if (depth2 < 0)
                depth2 = 0, error = true, token.classList.add("soullbf-error");
            } else
        ...
        if (depth2 > 0)
          error = true, token.classList.add("soullbf-error");
      ...
      iStep() {
        ...
        while (cond) {
          ...
          switch (instruction) {
            ...
            case "[":
              ...
              if (i3.match(/^\[([^><+-.,[\]]*)\-([^><+-.,[\]]*)\]/) && (vm.config.dataType !== 99 || curr > 0) || i3.match(/^\[([^><+-.,[\]]*)\+([^><+-.,[\]]*)\]/) && (vm.config.dataType !== 99 || curr < 0))
        ...
        if (vm.breakpoints.has(vm.codePointer))
          throw "break";
        vm.$nextTick(() => {
          let mt = vm.$el.querySelector(".soullbf-memory-table"), mp = mt.querySelector(".soullbf-pointer");
          if (mt && mp)
